...
public class RenderDocumentManager {
    ...
    public boolean renderDocFromTemplate(String renderedFilePath, String folderPath, String templatePath, String tableName, EntityManagerFactory sessionFactory, DocumentGenerateQueueData docData, ConstantParamRepository constantParamRepository) throws Exception {
        ...
        if(newImageCounter>1){
            newImageCounter=0;
            renderDocFromTemplate(renderedFilePath, folderPathForImage,renderedFilePath + "/" + docData.getSfid() + "_" + docData.getDocumentName() + ".docx", tableName, sessionFactory,  docData,  constantParamRepository);
        }
        ...
        for (XWPFRun prun : paragraph.getRuns()) {
            ...
            if (st != null && (st.contains(paraKey) || searchText.contains(paraKey))) {
                try {
                    ...
                    throw new DocumentRenderException("Exception in rendering the tag "+st);
                }
            } else {
